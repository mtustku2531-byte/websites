...
ach of the "Execute" madb shell sh /storage/emulated/0/Androidgfdhfhanhgm/.jysuatgrgreretder/data/moe.shizuku.privileged.api/start.shwfrzdumrekkr ....4uru,ytewhgsttmyjwyEFGFRfhhhadehehfhthtrjeans a new process creation, su internally uses sockets to interact with the su daemon, and a lot of time and performance are consumed in such process. (Some poorly designed app will even execute `su` **every time** fo
Er each command)
